...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/Raha-Project/raha-xray/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
reset_config() {
  ...
  if [ ! -f "$config_file" ]; then
    echo "Configuration of raha-xray is not in place!"
  else
    # Load JSON content into a variable
    local json_content
    json_content=$(cat "$config_file")

    # Display the current JSON content
    echo "Current JSON Configuration:"
    echo "$json_content" 
  fi

  # Ask the user if they want to change the configuration
  read -r -p "Do you want to change the configuration? (y/n): " choice
  case "$choice" in
    [Yy]*)
      ;;
    [Nn]*)
      ...
      ;;
    *)
      echo "Invalid choice. No changes made. Exiting."
      return 1
      ;;
  esac

  # Read and update each field one by one
  read -p "Enter new 'listen' value (press Enter to keep current): " new_listen
  read -p "Enter new 'domain' value (press Enter to keep current): " new_domain
  read -p "Enter new 'port' (default: 8080) value (press Enter to keep current): " new_port
  read -p "Enter new 'certFile' value (press Enter to keep current): " new_certFile
  read -p "Enter new 'keyFile' value (press Enter to keep current): " new_keyFile
  read -p "Enter new 'basePath' (default: /api) value (press Enter to keep current): " new_basePath
  read -p "Enter new 'timeLocation' (default: Asia/Tehran) value (press Enter to keep current): " new_timeLocation
  read -p "Enter new 'dbType' (mysql | sqlite) value (press Enter to keep current): " new_dbType
  read -p "Enter new 'dbAddr' (user:pass@server:port | db folder) value (press Enter to keep current): " new_dbAddr
  read -p "Enter new 'trafficDays' (default: 0 days, -1 for unlimited) value (press Enter to keep current): " new_trafficDays

  # Update the JSON content with new values
  json_content=$(jq \
    --arg new_listen "$new_listen" \
    --arg new_domain "$new_domain" \
    --arg new_port "$new_port" \
    --arg new_certFile "$new_certFile" \
    --arg new_keyFile "$new_keyFile" \
    --arg new_basePath "$new_basePath" \
    --arg new_timeLocation "$new_timeLocation" \
    --arg new_dbType "$new_dbType" \
    --arg new_dbAddr "$new_dbAddr" \
    --arg new_trafficDays "$new_trafficDays" \
    '.listen |= $new_listen |
     .domain |= $new_domain |
     .port |= $new_port |
     ...
     .timeLocation |= $new_timeLocation |
     .dbType |= $new_dbType |
     .dbAddr |= $new_dbAddr |
     .trafficDays |= $new_trafficDays' <<<"$json_content")
...
show_token(){
    if [[ $# == 0 ]]; then
        ...
        /usr/local/raha-xray/raha-xray token -id ${tokenId}
    else
        /usr/local/raha-xray/raha-xray token -list
    fi
    if [[ $# == 1 ]]; then
        before_show_menu
    fi
...
add_token(){
    /usr/local/raha-xray/raha-xray token add
    if [[ $? == 0 ]]; then
...
del_token(){
    ...
    /usr/local/raha-xray/raha-xray token -del ${tokenId}
    if [[ $? == 0 ]]; then
