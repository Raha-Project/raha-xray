...
func (s *InboundService) RebuildByApi(tx *gorm.DB, ids []uint, delFirst bool) error {
	...
	err = tx.Model(model.Inbound{}).Preload("Config").Preload("ClientInbounds").Where("inbounds.id in ?", ids).Find(&inbounds).Error
	if err != nil {
		return err
	}
	...
	for _, inbound := range inbounds {
		...
		if err == nil {
			logger.Debug("Inbound rebuilded by api:", inbound.Tag)
		} else {
			return err
		}
	}
