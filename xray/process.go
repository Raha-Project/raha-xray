...
func (p *process) refreshApiServer(configs *Config) {
	...
	if len(Env_API) > 0 {
		...
		for _, inbound := range configs.InboundConfigs {
			if inbound.Tag == "api" {
				addr := string(inbound.Listen)
				p.apiServer = fmt.Sprintf("%s:%d", strings.Trim(addr, "\""), inbound.Port)
				break
			}
		}
	}
